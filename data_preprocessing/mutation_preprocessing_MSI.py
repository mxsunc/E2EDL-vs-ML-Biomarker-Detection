...
path = '...'
tcga_maf = pickle.load(open(path+'controlled_allmut_consensus_multi_tcga_maf_table_20.pkl', 'rb'))
samples = pickle.load(open(path+ 'controlled_allmut_multi_MSI_consensus_sample_table.pkl', 'rb'))
panels= pickle.load(open(path+ 'controlled_allmut_consensus_multi_tcga_panel_table_20.pkl', 'rb'))

samples = samples.loc[~pd.isna(samples['PCR'])]
...
msi_dict = {'nonMSIH': 'nonMSIH', 'MSIH': 'MSIH'}
samples['PCR'] = samples.PCR.apply(lambda x: msi_dict[x])
samples

A = samples.PCR.astype('category')
...
with open(path +'controlled_allmut_multi_msi_consensus_data_finished_20.pkl', 'wb') as f:
