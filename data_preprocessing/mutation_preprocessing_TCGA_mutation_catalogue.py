...
path = '...'
tcga_maf = pickle.load(open(path+'tcga_maf_table_id_mutation_catalogue.pkl', 'rb')) # or SBS data

tcga_dfs = []
tcga_maf["Patient_ID"] = [x[:12] for x in tcga_maf['Tumor_Sample_Barcode'].tolist()]
patients = tcga_maf['Patient_ID'].unique().tolist()
for patient in patients:
    df = tcga_maf[tcga_maf["Patient_ID"]==patient]
    tcga_dfs.append(df)
...
for dataframe in tcga_dfs:
    ...
    sbs_catalogue["Patient_ID"] = dataframe.Patient_ID.unique()[0]
    dict_list.append(sbs_catalogue)
sbs_catalogue_df = pd.DataFrame(dict_list)
sbs_catalogue_df.to_csv(path+"tcga_sbs_mutation_catalogues_norm.csv",index=False)
# %%
dict_list = []
for dataframe in tcga_dfs:
    ...
    id_catalogue["Patient_ID"] = dataframe.Patient_ID.unique()[0]
    dict_list.append(id_catalogue)
id_catalogue_df = pd.DataFrame(dict_list)
id_catalogue_df.to_csv(path+"tcga_id_mutation_catalogues_norm.csv",index=False)