#%%
"""
CNV Autoencoder (GISTIC genes) - 5-Fold CV Prediction Export (Revision)

Trains an AE on CNV data and exports predictions in a standardized format
...
Output:
- predictions_CNV_AE_all_folds.csv: Per-patient predictions for all cohorts
"""
...
for j in range(5):
    ...
    y_pred_test = model.predict((X_test_z,), verbose=0)[1]
    ...
    y_pred_cptac = model.predict((X_cptac_z,), verbose=0)[1]
    ...
    encoder.save_weights(os.path.join(WEIGHTS_DIR, f"HRD_ae_encoder_weights_fold{j+1}.h5"))
...
output_dir = "predictions_CNV_AE_gistic/"
os.makedirs(output_dir, exist_ok=True)

output_path = os.path.join(output_dir, "predictions_CNV_AE_gistic_all_folds.csv")
...
for cohort in predictions_df['cohort'].unique():
    ...
    cohort_path = os.path.join(output_dir, f"predictions_CNV_AE_gistic_{cohort}.csv")
...
print(f"{'='*60}")
print(f"python run_evaluation_pipeline.py --input {output_path} --output-dir evaluation_results_CNV_AE_gistic/")
