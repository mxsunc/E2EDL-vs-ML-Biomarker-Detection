...
for j in range(5):
    ...
    cnv = CNVVAE(INPUT_DIM, LATENT_DIM, DROPOUT, L2_WEIGHT, beta_kl=0.01)
    vae = cnv.model

    x_b, y_b = next(iter(train_ds))
    y_batch  = y_b["hrd_pred"].numpy().ravel()

    raw_pred = vae.predict(x_b, batch_size=64)[1].ravel()
    ...
    vae.save_weights(f".../weights/fold{j+1}_vae_full_weights_seg_512.h5")
    cnv.encoder.save_weights(f".../weights/fold{j+1}_vae_encoder_weights_seg_512.h5")
    cnv.preclassifier.save_weights(f".../weights/fold{j+1}_vae_preclassifier_weights_seg_512.h5")

    recon_test,y_pred = vae.predict(X_test)
    ...
    recon_test,y_pred = vae.predict(X_cptac)
    y_pred_binary = (y_pred >= best_threshold).astype(int)