...
for j in range(5):
    ...
    labels_all_cptac = np.array(sample_df_cptac['HRD_Binary'].tolist())c = np.array(sample_df_cptac['HRD_Binary'].tolist())#samples['class'][:, 1][:, np.newaxis]
    ...
    cnv_input = tf.keras.Input(shape=(N_CN_FEATURES,), name="cnv_vec")
    ...
    cnv_encoder.load_weights(".../weights/fold"+str(j+1)+"_ae_encoder_weights_seq_512.h5")
    ...
    x = Concatenate()([mil_emb, cnv_emb, ctx_mil, ctx_cnv]) #[mil_emb, cnv_emb, ctx_mil, ctx_cnv]
    ...
    callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_BE', min_delta=0.002, patience=5, mode='max', restore_best_weights=True,start_from_epoch = 1)]
    ...
    y_pred_binary = (y_pred_cptac >= best_threshold).astype(int)
    binarized_labels = y_label_cptac[val_cptac]