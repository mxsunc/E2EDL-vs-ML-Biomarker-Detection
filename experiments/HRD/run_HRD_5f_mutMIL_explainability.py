...
names_cancer_types = sample_df['HRD_status'].unique().tolist()
...
for cluster_index in range(len(cluster_dataframes)):
        ...
        plot_sequence_logos_for_cluster(five_prime_seqs, ref_seqs, alt_seqs, three_prime_seqs)
...
string_labels = sample_df["HRD_status"].tolist()
...
for label in cancer_colors.keys():
    indices = sample_df[sample_df["HRD_status"] == label].index.tolist()
